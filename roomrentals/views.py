...
class HomeView(DayArchiveView):
	...
	def my_view(request):
		my_objects = list(RoomRental.objects.filter(public=True))
		if not my_objects:
	...
	def get(self, request, *args, **kwargs):
		...
		qs = RoomRental.objects.filter(user__is_active=True, appointmentdate=todaydate).order_by("appointmenttime")
#		if todaydate = 
		return render(request, "roomrentals/home-feed.html", {'object_list':qs, 'previous':False, 'today':todaydate, 
...
class DayView(DayArchiveView):
	...
	def get_queryset(self, *args, **kwargs):
		...
		return RoomRental.objects.filter(user__is_active=True)
